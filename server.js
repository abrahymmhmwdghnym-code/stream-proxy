...
// ============================================
// 📝 إضافة إعلان نصي على الفيديو (Subtitle Track)
// ============================================

function addAdvertToM3U8(data, advertText, websiteUrl) {
    // 1️⃣ إنشاء ملف WebVTT للإعلان (يظهر طول الفيديو)
...
تابعنا الآن`;
    ...
    // 2️⃣ تحويل إلى Base64 عشان نضيفه مباشرة في M3U8
    const vttBase64 = Buffer.from(vttContent).toString('base64');
    
    // 3️⃣ إضافة مسار الترجمات في M3U8
...
app.get('/api/stream', async (req, res) => {
    ...
    try {
        ...
        // 🎨 إعدادات الإعلان
        // ============================================
        const advertText = 'لمشاهدة جميع المباريات تابع موقع إبراهيم لايف';
        ...
        if (isM3U8) {
            ...
            // 🔥 إضافة الإعلان على الفيديو
            data = addAdvertToM3U8(data, advertText, websiteUrl);
...
app.get('/', (req, res) => {
    res.type('text/html').send(`
        ...
        <head>
            ...
            <title>📡 وكيل البث المتقدم - إبراهيم لايف</title>
            <style>
...
║     • يظهر طوال مدة البث                                    ║
║     • قابل للتشغيل في أي مشغل يدعم الترجمات                 ║
║                                                                ║
╚════════════════════════════════════════════════════════════════╝
    `);
});
